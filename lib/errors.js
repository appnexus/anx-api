...
function ApiError(req, rawRes) {
	...
	if (_.isObject(rawRes)) {
		// Traverse through general API JSON Response
		if (rawRes.body && rawRes.body.response) {
			res = rawRes.body.response; // res is raw api response
		} else if (rawRes.response) {
			res = rawRes.response; // res is body
		} else {
			res = rawRes;
		}

		// Extract values from object - duck type check
		if (res.hasOwnProperty('error_id')) { // Api Response
			id = res.error_id;
			code = res.error_code;
			message = res.error;
			description = res.error_description;
		} else if (res.hasOwnProperty('id')) { // Simple Object
			id = res.id;
			code = res.code;
			message = res.message;
