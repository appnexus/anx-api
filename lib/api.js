...
Api.TargetError = function(message) {
    this.name = 'TargetError';
    this.message = (message || '');
};
Api.TargetError.prototype = new Error();

Api.NotAuthorizedError = function(message) {
    this.name = 'NotAuthorizedError';
    this.message = (message || '');
};
Api.NotAuthorizedError.prototype = new Error();
		...
		return _self._config.request(opts).then(function (res) {
			...
			if (res.statusCode === 401) {
				if (_.isObject(res.body)) {
					message = res.body.response.error;
				} else {
					message = 'Authentication failed';
				}
				return reject(new Api.NotAuthorizedError(message));
			}
			return resolve(res);
		});
	});
