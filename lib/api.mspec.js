...
describe('Api', function () {
	...
	describe('request', function () {
		...
		describe('opts', function () {
			...
			describe('validation', function () {
				...
				function expectValidationError(opts, expected, done) {
					api.request(opts).then(function () {
				...
				_.each({
					uri: [
						{ value: 'creative' },
						// { value: undefined, message: 'Parameter \'url\' must be a string, not object' },
						{ value: null, message: 'Parameter \'url\' must be a string, not object' },
						{ value: undefined, message: 'Parameter \'url\' must be a string, not undefined' }
					],
					startElement: [
						{ value: null },
						{ value: undefined },
					...
					describe(param, function () {
						tests.forEach(function (test) {
							...
							opts[param] = test.value;
							if (test.message) {
								it(param + ' should not accept ' + test.value, function (done) {
									expectValidationError(opts, test.message, done);
								});
							} else {
								it(param + ' should accept ' + test.value, function (done) {
									api.request(opts).then(function () {
										if (test.uriContains) {
											assert(_.contains(reqOpts.uri, test.uriContains), 'Url (' + reqOpts.uri + ') did not contain: ' + test.uriContains);
										}
										done();
									}).catch(function (err) {
										done(err);
			...
			describe('Errors', function () {
				...
				describe('NotAuthorizedError', function () {
					...
					it('should reject with NotAuthorizedError', function (done) {
						api.getJson('user').then(function () {
							return done(new Error('Did not catch 401'));
						}).catch(function (err) {
							if (!(err instanceof Api.NotAuthorizedError)) {
								return done(new Error('Did not catch NotAuthorizedError'));
							}
							done();
						});
					});
	...
	describe('JS Error Types', function () {
		...
		describe('Api.buildError', function() {
