...
describe('AnxApi', function() {
			...
			describe('json default', function() {
				...
				describe('header overrides', function() {
					...
					it('should allow overriding Content-Type', function(done) {
						api.request({ method: 'GET', headers: { 'Content-Type': 'application/json' } }).then(function(opts) {
							assert.equal(opts.headers.Accept, 'application/json', 'bad or missing Accept');
	...
	describe('#getBy', function() {
		...
		it('should get single param', function(done) {
			requestStub.resolves({ body: { response: { count: 1, user: { name: 3 }, dbg_info: { output_term: 'user' } } } });
			api.getBy('name', 3, 'test').then(function(res) {
				assert.deepEqual(requestStub.firstCall.args[0].params, { name: 3 });
				assert.deepEqual(res.body, {
					response: {
						count: 1,
						dbg_info: {
		...
		it('should get multi ids', function(done) {
			requestStub.onCall(0).resolves({ body: { response: { users: [{ id: 1 }, { id: 2 }], dbg_info: { output_term: 'users' } } } });
			requestStub.onCall(1).resolves({ body: { response: { user: { id: 3 }, dbg_info: { output_term: 'user' } } } });
			api.getBy('name', [1, 2, 3], 'test').then(function(res) {
				assert.deepEqual(requestStub.firstCall.args[0].params, { name: '1,2' });
				assert.deepEqual(requestStub.secondCall.args[0].params, { name: '3' });
				assert.deepEqual(res.body, {
					response: {
						count: 3,
						dbg_info: {
	...
	describe('#getAll', function() {
		...
		it('should ', function(done) {
			requestStub.onCall(0).resolves({ body: { response: { status: 'OK', count: 3, num_elements: 2, users: [{ id: 1 }, { id: 2 }], dbg_info: { output_term: 'users' } } } });
			requestStub.onCall(1).resolves({ body: { response: { status: 'OK', count: 3, num_elements: 2, user: { id: 3 }, dbg_info: { output_term: 'user' } } } });
